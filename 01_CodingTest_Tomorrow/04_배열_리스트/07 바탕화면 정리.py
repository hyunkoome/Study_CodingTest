...
def solution(wallpaper):
    sero_x_list = []
    garo_y_list = []
    for sero_x, wall in enumerate(wallpaper):
        for garo_y, value in enumerate(wall):
            if value == '#':
                sero_x_list.append(sero_x)
                garo_y_list.append(garo_y)

    answer = [min(sero_x_list), min(garo_y_list), max(sero_x_list) + 1, max(garo_y_list) + 1]
    return answer
...
if __name__ == "__main__":
    ...
    for quiz_dict in quiz_dict_list:
        res = solution(wallpaper=quiz_dict['wallpaper'])
