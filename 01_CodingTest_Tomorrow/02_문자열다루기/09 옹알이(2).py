...
def solution(babbling):
    ...
    for word in babbling:
        for ongali in valid:
            if ongali*2 in word:  # 동일한 단어 연속으로 있는지 체크
                break
            word = word.replace(ongali, " ")

        if not word.strip():  # word.strip() == "" , word 가 모두 발음 가능한 단어로 바꿔서 공백이 됬다면
            answer += 1
    return answer
...
if __name__ == '__main__':
    ...
    for quiz_dict in quiz_dict_list:
        res = solution(babbling=quiz_dict['babbling'])
